...
**No separate timeline logic per editor.** Instead, `@storyteller/exporters` provides **translators** that turn the same sequence + resolved asset paths into **target-specific packages**:
...
The desktop app also writes **`manifest.json`** and **`README.txt`** next to the timeline files. See `docs/desktop-export.md`.
